...
contract InscriptionFactoryTest is Test {
    ...
    function testDeployInscription() public {
        ...
        (address deployedAddr, uint totalSupply, uint perMint) = factory.tokens(
            tokenAddr
        );
        assertEq(
            deployedAddr,
            tokenAddr,
            "Deployed address should match the token address"
        );
    ...
    function testMintInscription() public {
        ...
        assertEq(
            balanceAfterMint,
            PER_MINT,
            "Balance should match per mint amount after mint"
        );
        ...
        assertEq(
            balanceAfterSecondMint,
            PER_MINT * 2,
            "Balance should match twice the per mint amount after second mint"
        );
    }
